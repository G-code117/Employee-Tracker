      ...
      .prompt([
        {
          type: "list",
          name: "departmentId",
          message: "Which department do employees belong to?",
...
function viewEmployeesByManager() {
  ...
  pool.query(sqlQuery, (err, results) => {
    ...
    const managerChoices = results.rows.map(
      ...
      .prompt([
        {
          type: "list",
          name: "managerId",
          message: "Employee reporting structure views",
                ...
                .then(({ managerId }) => {
                  ...
                  pool.query(
                    ...
                    (err, result) => {
                      ...
                      console.log(
                        `Employee added: ${employee.first_name} ${employee.last_name}`
                      );
                      loadMainMenu();
                    }
                  );
...
function updateEmployeeRole() {
  pool.query("SELECT * FROM employee", (err, roleResults) => {
    ...
    const employeeChoices = roleResults.rows.map(
      ({ id, first_name, last_name }) => ({
        name: `${first_name} ${last_name}`,
        value: id,
      })
      ...
      .prompt([
        {
          type: "list",
          name: "employeeId",
          message: "Which employees to update?",
          choices: employeeChoices,
        },
      ])
      .then(({ employeeId }) => {
      ...
      .then(({ employeeId }) => {
        pool.query(
          ...
          (err, managerResults) => {
            ...
            const managerChoices = managerResults.rows.map(
              ({ id, first_name, last_name }) => ({
                name: `${first_name} ${last_name}`,
                value: id,
              })
    ...
    inquirer
      .prompt([
        ...
        },
        {
          type: "confirm",
          name: "confirm",
          message: "Remove from this department?",
          default: false,
        },
      ])
